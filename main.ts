...
namespace artecrobo {
    ...
    let speedM1 = 1023;
    let speedM2 = 1023;
    let state = DCmotion1.Brake;

    // Move DC motor
    //% blockId=artec_move_dc_motor group="Motor"
    //% block="DC motor %_connector| motion: %_motion"
    export function moveDCMotor(_connector: connectorDCMotor1, _motion: DCmotion1): void {
        switch(_motion) {
    ...
    export function setSpeedDCMotor(_connector: connectorDCMotor1, _speed: number): void {
        if (_speed < 0)     { _speed = 0; }
        ...
        if (state == DCmotion1.Forward || state == DCmotion1.Backward) {
